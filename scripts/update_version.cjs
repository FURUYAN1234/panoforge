...
try {
  ...
  // 2. index.html
  console.log(`[2/3] Updating index.html...`);
  const htmlContent = fs.readFileSync(targetFiles.indexHtml, 'utf8');
  const updatedHtml = htmlContent.replace(
    /360° AI Panorama Generator v\d+\.\d+\.\d+(-alpha)?/g,
    `360° AI Panorama Generator v${newVersion}`
  ).replace(
    /v\d+\.\d+\.\d+(-alpha)?<\/span>/g,
    `v${newVersion}</span>`
  ).replace(
    /v\d+\.\d+\.\d+(-alpha)?\s*<\/span>/g,
    `v${newVersion}</span>`
  );
  fs.writeFileSync(targetFiles.indexHtml, updatedHtml, 'utf8');

  // 3. README.md
  console.log(`[3/3] Updating README.md...`);
